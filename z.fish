...
function grep_commandline_filter
    ...
    set --local current_cmd_token (commandline --current-token)
    set --local last_cmd_token (commandline --current-token)
    if begin; [ $num_cmd_words = 1 ]; or [ $num_cmd_words = 2 -a "$current_cmd_token" = "$last_cmd_token" ]; end
        cat
    else
...
function __fish_complete_j
    ...
    set --local num_cmd_words  (commandline | wc --words)
    set --local last_token     (commandline --tokenize | tail --lines 1)
    set --local current_token  (commandline --current-token)
    set --local last_cmd_char  (commandline | tail --bytes 2 | head --bytes 1)

    begin
        if begin; [ $num_cmd_words = 1 ]; or [ $num_cmd_words = 2 -a "$last_token" = "$current_token" ]; end
            __fish_complete_cd
            echo -n (z --complete "")/\n
        end
        echo -n $unique_z_words\n
    end | sed 's/^ //' | grep_commandline_filter
end
...
function z -d "Jump to a recent directory"
    ...
    # tab completion
        ...
        if [ "$argv[1]" = "--complete" ]
            ...
            set -l tempfile (mktemp $datafile.XXXXXX)
            set -l tempfile2 (mktemp $datafile.XXXXXX)
            test -f $tempfile; or return
            set -l target (awk -v t=(date +%s) -v list="$list" -v typ="$typ" -v q="$fnd" -v tmpfl="$tempfile" -v tmpfl2="$tempfile2" -F"|" '
